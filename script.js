...
var clickedColour = standardizeColor(this.style.backgroundColor);

		if (clickedColour === standardizeColor(pickedColor)){
resultMessage.textContent= "You Win!";
resetButton.textContent= "Play Again";
changeColors(clickedColour);
banner.style.backgroundColor= clickedColour;
...
pickedColor= standardizeColor(chooseColor());
colourToGuess.textContent=pickedColor;

for (var i =0; i<circles.length;i++){
	circles[i].style.backgroundColor=standardizeColor(colours[i]);
